...
int   search_env_content(t_env *env, t_list *name)
{
	int	i = 0;
...
char **ft_split (char *str)
{
    ...
    int j = 0;
    int tmp = 0;
    char **tab = malloc(sizeof(char *) * ft_strlen(str) + 1); // CHECK TES MALLOCS AMOOUUURRRR -> Il faudrait que tu malloc d'une taille qui représente la réelle taille de ton tableau de char * 
    ...
    while (str[i] != '\0')
    {
        ...
        else if (str[i] == '"')
        {
            ...
            while(str[i] != '"')
            {
                ...
                if (str[i] == '\0')
                {
                    printf("Jte vois enculer y a pas de segfault ici\n");
                    exit(-81);
                }
            }
            i = i + 1;
